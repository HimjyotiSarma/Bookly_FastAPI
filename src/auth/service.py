...
class UserService:
    async def get_user(self, email: str, session: AsyncSession):
        try:
            if email is None:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Please provide Email Id")
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error getting User : {str(e)}")
    ...
    async def create_user(self, user_data: UserCreateModal, session: AsyncSession):
        try:
            ...
            if 'role' in user_data_dict:
                raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail="Manual Role Implementation is not Allowed")
